...
from models.networks import NGPA
...
class NeRFSystem(LightningModule):
    def __init__(self, hparams):
        ...
        self.model = NGPA(scale=self.hparams.scale, vocab_size=self.hparams.vocab_size, rgb_act=rgb_act, dim_a = self.hparams.dim_a)
...
if __name__ == '__main__':
    ...
    ckpt_cb = ModelCheckpoint(dirpath=f'ckpts/NGPA/{hparams.dataset_name}/{hparams.exp_name}',
                              filename='{epoch:d}',
    ...
    logger = TensorBoardLogger(save_dir=f"logs/NGPA/{hparams.dataset_name}",
    ...
    if not hparams.val_only: # save slimmed ckpt for the last epoch
        ckpt_ = \
            slim_ckpt(f'ckpts/NGPA/{hparams.dataset_name}/{hparams.exp_name}/epoch={hparams.num_epochs-1}.ckpt',
                      save_poses=hparams.optimize_ext)
        torch.save(ckpt_, f'ckpts/NGPA/{hparams.dataset_name}/{hparams.exp_name}/epoch={hparams.num_epochs-1}_slim.ckpt')
